...
# --- GLOBAL CONSTANTS ---
X_EQUITY_TICKERS = ["XLK", "XLY", "XLP", "XLE", "XLV", "XLI", "XLB", "SMH", "SOXX", "XLU", "XLC", "XLF", "XBI", "XME", "XHB", "XSD", "XRT", "GDX", "XAR", "XLB", "XLRE", "IWM", "IWF", "IWD", "IWO", "XNTK"]
...
def fetch_sofr_fred(start_date="2008-01-01"):
    """Fetch SOFR data directly from FRED CSV endpoint"""
    try:
        url = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=SOFR"
        df = pd.read_csv(url, index_col=0, parse_dates=True)
        df = df[df.index >= start_date]
        df = df.rename(columns={'SOFR': 'SOFR_ANNUAL'})
        return (df / 100).ffill()  # Convert to decimal and forward fill
...
def seed_dataset_from_scratch():
    ...
    # Fetch SOFR directly from FRED instead of using pandas_datareader
    sofr = fetch_sofr_fred("2008-01-01")
    if sofr is not None:
        master_df['SOFR_ANNUAL'] = sofr
    else:
